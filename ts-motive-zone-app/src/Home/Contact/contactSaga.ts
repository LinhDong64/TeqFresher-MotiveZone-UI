...
export function* checkEmail(action: any) {
    const data: object = yield call(validateFunction.validateEmail,
        action.payload.data);
    yield put(contactActions.onChangeEmail(data));
}

export function* checkMessage(action: any) {
    const data: object = yield call(validateFunction.validateMessage,
        action.payload.data);
    yield put(contactActions.onChangeMessage(data));
}

export function* sendForm(action: any) {
    const data: object = yield call(callAPI.sendContactForm,
        action.payload);
